...
def train_model(train: Train, pf: Featureset("hugging_features")) -> Any:
    ...
    buffer_size = 2
    model.fit(train_dataset.shuffle(buffer_size).batch(16), validation_data=test_dataset.shuffle(buffer_size).batch(16), epochs=1,
              batch_size=16)
    loss, accuracy = model.evaluate(train_dataset.shuffle(buffer_size).batch(16))
    train.log_metric("Training Accuracy", accuracy)
    train.log_metric("Training Loss", loss)

    test_loss, test_accuracy = model.evaluate(test_dataset.shuffle(buffer_size).batch(16))
